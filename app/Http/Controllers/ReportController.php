...
class ReportController extends Controller
{
    ...
    public function productExpiryAlert()
    {
        ...
        if($role->hasPermissionTo('product-qty-alert')){
            ...
            $lims_product_data = Product::select('name','code', 'image', 'qty', 'expiry_date')->where('is_active', true)->whereDate('expiry_date', '<=', $two_months_from_now)->get();
            return view('report.expr_alert_report', compact('lims_product_data'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
